...
function ProblemPage() {
  ...
  //update problem when ever url param changes
  useEffect(()=>{
    if(id && PROBLEMS[id]){
      setCurrProblemID(id);
  ...
  const handleLanguageChange = (e)=>{
    ...
    setCode(PROBLEMS[currProblemId].starterCode[newLang]);
    setOutput(null);
  }
  ...
  const handleRunCode = async()=>{
    ...
    if(result.success){
      const expectedOutput = currProblem.expectedOutput[selectedLanguage];
  ...
  return <div className="h-screen  bg-base-100 flex flex-col">
    ...
    <div className="flex-1">
      <PanelGroup direction="horizontal">
        ...
        <PanelResizeHandle className="w-2 bg-base-300 hover:bg-emerald-600
        transition-colors cursor-col-resize"/>

        {/*Right Panel- CodeEditor and Output Window */}
        <Panel defaultSize={60} minSize={30}>
          <PanelGroup direction="vertical">

            {/*Top Panel - CodeEditorPanel*/}
            ...
            <PanelResizeHandle className="h-2 bg-base-300 hover:bg-emerald-600 
            transition-colors cursor-row-resize"/>

            {/*Bottom Panel - Building OutputPanel*/}
